...
// stubStore is an in-memory hand-rolled stand-in for `keep serve`. It implements
...
func newStubServer(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("POST /api/assertions", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(body.Pins) == 0 {
			writeErr(w, http.StatusBadRequest, "keep: at least one pin is required")
			return
		}
		s.mu.Lock()
		s.seq++
	...
	mux.HandleFunc("GET /api/assertions/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		s.mu.Unlock()
		if !ok {
			writeErr(w, http.StatusNotFound, "keep: not found")
			return
		}
