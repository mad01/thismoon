...
import (
	...
	"github.com/mad01/thismoon/services/keep/internal/client"
)
...
func registerTools(s *mcp.Server, h *handlers) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Record a one-sentence assertion about how a system behaves and ground it in evidence pins. " +
			...
			"Each pin captures a line range in a repo working tree, hashed the moment you assert; `keep_check` re-hashes them later and flips the assertion stale if the pinned code changed. " +
			...
			"Keep the returned id — it is the handle for keep_get / keep_retract / keep_check.",
	}, h.handleAssert)
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "List assertions, newest first, to see what past sessions already established before deriving it again. " +
			...
			"Returns each assertion's id, statement, kind, confidence, and status — follow up with keep_get for the full record including pins.",
	}, h.handleQuery)
...
type idInput struct {
	ID string `json:"id" jsonschema_description:"assertion id returned by keep_assert"`
}
