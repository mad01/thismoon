// Package client is the HTTP client for a running `keep serve`. Both the MCP
...
// Client talks to a running `keep serve` over its localhost HTTP API.
type Client struct {
	baseURL string
	http    *http.Client
}
...
func (c *Client) do(method, path string, body, out any) error {
	...
	if err != nil {
		return fmt.Errorf(
			"keep serve not reachable at %s — is the t-man agent running? (t-man status keep): %w",
			c.baseURL,
			err,
		)
	}
	...
	if res.StatusCode >= 400 {
		...
		if apiErr.Error != "" {
			...
			// "keep: not found"); surface them as-is to avoid double prefixing.
			return errors.New(apiErr.Error)
		}
		return fmt.Errorf("keep: serve returned %d", res.StatusCode)
	}
	if out != nil {
