...
// stubServer runs a hand-rolled handler standing in for `keep serve`, so these
...
func TestDoReportsUnreachableServe(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "t-man status keep") {
		t.Errorf("error = %q, want it to mention 't-man status keep'", err.Error())
	}
}
