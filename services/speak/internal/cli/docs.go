...
var docsCmd = &cobra.Command{
	Use:   "docs",
	Short: "Print the operating doc for agents and humans debugging speak",
	Long: `Print the embedded operating doc: how speak runs, where its state lives, the
...
the same defaults the binary runs with, so it cannot drift from the code.`,
	RunE: func(cmd *cobra.Command, _ []string) error {
		doc, err := agentdoc.Render(speak.OperatingDoc, speak.Facts())
		if err != nil {
			return err
		}
