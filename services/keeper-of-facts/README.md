...
proves it. When the code moves on, keeper-of-facts (kof) tells you which of those notes to look
at again.
...
ends. kof makes those findings persistent and keeps them honest: each one is a
...
stays fresh. If it changed, the assertion goes stale and kof names the pin that
...
`~/code/bin/kof`, registers the `kof serve` agent with t-man, adds the
`kof.this` route, and wires the Claude Code tools.
...
$ kof list --subject repo:mad01/thismoon
...
```bash
kof assert --kind <kind> --subject <key> --statement "<sentence>" \
