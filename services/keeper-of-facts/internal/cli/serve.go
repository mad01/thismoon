...
Typically run as a background service:
  t-man add --name keeper-of-facts -- kof serve --port 7431`,
	RunE: runServe,
}
...
func runServe(_ *cobra.Command, _ []string) error {
	...
	go watchStore(st)
	log.Printf("kof: serving %s on http://localhost:%d", flagWorkdir, flagPort)
	log.Printf(
		"kof: register as a background service with: t-man add --name keeper-of-facts -- kof serve --port %d",
		flagPort,
	)
...
func watchStore(st *store.Store) {
	...
	for range t.C {
		...
		if err != nil {
			log.Printf("kof: reload store: %v", err)
			continue
		}
		if reloaded {
			log.Printf("kof: store changed on disk, reloaded")
		}
	}
}
