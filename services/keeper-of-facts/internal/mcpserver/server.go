...
type Config struct {
	...
	// Checks builds the diagnostics behind the kof_doctor tool, against the
	// same resolved flags the rest of the CLI uses. It is required: the
	// instructions block advertises kof_doctor to every client, so a server
...
func New(version string, cfg Config) (*mcp.Server, error) {
	if cfg.Checks == nil {
		return nil, errors.New("mcpserver: no doctor checks; kof_doctor is advertised to clients and must be registered")
	}
