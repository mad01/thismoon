...
kof is not a general note store: one sentence per assertion, at least one
...
`kof.this` is read-only: writing, checking, and retracting go through the
MCP or the CLI. kof does not decide whether a stale assertion still holds or
