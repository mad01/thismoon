...
internal/
  ...
  repo/            repo discovery + host routing
    ...
    catalogspec/   reads the catalog descriptor a repo may carry at its root
                    (catalog-info.yaml, service-info.yaml, or a
                    .csl-catalog.yaml pointer)
  picker/          csl's own fuzzy-finder picker (tcell), used by `csl repo`
...
**Catalog identity.** At discovery, the walker reads a repo's catalog
descriptor from its root: `.csl-catalog.yaml` first (a csl-only pointer file
whose single `descriptor:` key names a repo-relative path, for a monorepo or
non-standard layout where the descriptor isn't at the root), else
`catalog-info.yaml` (Backstage's name), else `service-info.yaml` (the catalog
service's name). The first Component entity wins; `backstage.io/*` and
`catalog.mad01/*` apiVersion groups are accepted at any version, and a
missing apiVersion is treated as the catalog service's default. csl keeps
only `metadata.name` (component), `spec.owner` (owner), and `spec.system`
(system), on `finder.Repo.Catalog`. A malformed descriptor is dropped
silently so one repo's YAML never fails the walk; `csl doctor`
(`catalog-descriptors`) surfaces it. Nothing is cached: the descriptor is
re-read on every discovery walk. See docs/adr/0017.
...
- **`csl search <pattern>`**: search across all indexed repos. `--repo/-r`, `--lang/-l`, `--file/-f`, `--limit` (50), `--output-mode/-o` (files_with_matches\|content), `--context-lines/-C`, `--case-sensitive`, `--reindex` (synchronous re-index first), `--json`, `--toon`. `--serve` starts the search daemon in the foreground (there is no separate `csl serve`); `--stop` stops it. Content mode prints ripgrep `--heading` style blocks (`repo/path` header, `LINE:COL:match` with the 1-based match column as in ripgrep `--column`, `LINE-context`, `--` between gaps), merging nearby hits into one block per context window via `search.Blocks`, the same grammar and code the `csl_search` text format uses except that the MCP text form prints `LINE:match` without the column (`search.RenderOptions.Column`); `--json` stays one entry per line.
...
- **`csl docs`**: print the embedded operating doc (`operating.md`, rendered with the resolved paths). `--claude-md` prints the embedded CLAUDE.md section (`claude-md.md`) instead; `claude_md_test.go` holds it byte-identical to the fenced block under "Add this to your CLAUDE.md" in `README.md` and checks it names every tool `csl mcp` registers, so edit the README block and the embed together.
- **`csl shell-init <zsh|bash>`**: print the `repo` and `repo-sync` shell functions for `eval "$(csl shell-init zsh)"`. `shell_init_test.go` asserts the bodies match `[shell.functions]` in `recipes/csl/recipe.toml`, so change both or neither.
...
**Repo:**
- `csl_repo_lookup(name?, component?, owner?, system?)` → `{matches: [{name, path, remote?, host?, component?, owner?, system?}], dropped?: [{name, path, remote?, host?, component?, owner?, system?, reason}]}`. Every field is a case-insensitive regex; at least one is required, and all set fields must match. `component`/`owner`/`system` match the repo's catalog descriptor (`metadata.name`/`spec.owner`/`spec.system`); a repo with no descriptor never matches them, so `owner=".*"` still excludes it. When `matches` is empty the handler also matches the query against what discovery dropped (`cfg.DiscoverReposReport()`), so `dropped` with a `reason` means a filter hid the checkout; empty both means the repo isn't checked out locally, so don't guess a path.
