...
- **`csl search <pattern>`**: search across all indexed repos. `--repo/-r`, `--lang/-l`, `--file/-f`, `--limit` (50), `--output-mode/-o` (files_with_matches\|content), `--context-lines/-C`, `--case-sensitive`, `--reindex` (synchronous re-index first), `--json`, `--toon`. `--serve` starts the search daemon in the foreground (there is no separate `csl serve`); `--stop` stops it. Content mode prints ripgrep `--heading` style blocks (`repo/path` header, `LINE:match`, `LINE-context`, `--` between gaps), merging nearby hits into one block per context window via `search.Blocks`, the same grammar and code the `csl_search` text format uses; `--json` stays one entry per line.
