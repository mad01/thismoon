...
var docsCmd = &cobra.Command{
	Use:   "docs",
	Short: "Print the operating doc for agents and humans debugging csl",
	Long: `Print the embedded operating doc: how csl runs, where its state lives, the
...
the same defaults the binary runs with, so it cannot drift from the code.`,
	RunE: func(cmd *cobra.Command, _ []string) error {
		doc, err := agentdoc.Render(csl.OperatingDoc, csl.Facts())
		if err != nil {
			return err
		}
