...
func TestFilterOnly(t *testing.T) {
	repos := []finder.Repo{
		{Name: "org/a", Path: "/repos/a"},
		{Name: "org/b", Path: "/repos/b"},
	}

	if got := filterOnly(repos, "org/b"); len(got) != 1 || got[0].Path != "/repos/b" {
		t.Errorf("filterOnly by name = %v, want org/b only", got)
	}
	if got := filterOnly(repos, "/repos/a"); len(got) != 1 || got[0].Name != "org/a" {
		t.Errorf("filterOnly by path = %v, want org/a only", got)
	}
	if got := filterOnly(repos, "nope"); len(got) != 0 {
		t.Errorf("filterOnly no match = %v, want empty", got)
	}
}
...
func TestLocked(t *testing.T) {
	...
	if !Locked(dir) {
		t.Error("Locked should be true while the lock is held")
	}
	unlock()
	if Locked(dir) {
		t.Error("Locked should be false after unlock")
	}
}
