...
type hybridHit struct {
	...
	LexRank int    `json:"lex_rank"            jsonschema:"1-based rank in the lexical (zoekt) results; 0 if the file did not match lexically"`
	...
	SemRank  int     `json:"sem_rank"             jsonschema:"1-based rank in the semantic results; 0 if the file did not match semantically"`
...
type hybridSearchOutput struct {
	SemanticAvailable bool        `json:"semantic_available" jsonschema:"false when the semantic index/model is not ready; results are then lexical-only — see note"`
...
func registerHybridTools(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Search code by combining lexical (exact/regex) and semantic (meaning) search and fusing the two rankings with Reciprocal Rank Fusion. " +
			...
			"If the semantic index is not built (csl index --semantic-all), results degrade to lexical-only with semantic_available=false.",
	}, handleHybridSearch)
}
