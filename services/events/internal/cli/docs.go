...
var docsCmd = &cobra.Command{
	Use:   "docs",
	Short: "Print the operating doc for agents and humans debugging events",
	Long: `Print the embedded operating doc: how events runs, where its state lives, the
...
the same defaults the binary runs with, so it cannot drift from the code.`,
	RunE: func(cmd *cobra.Command, _ []string) error {
		doc, err := agentdoc.Render(events.OperatingDoc, events.Facts())
		if err != nil {
			return err
		}
