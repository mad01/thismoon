...
## Use it from Claude Code (the main way)

Ask in plain language — "remind me to call the dentist tomorrow at 9", "what are
my reminders", "cancel the standup one". Claude works out the time and calls the
`reminder_*` MCP tools (`create`, `list`, `get`, `edit`, `cancel`, `test`,
`fire`). The `/reminder` skill spells out the conversation patterns, but the
...
## The web view (read-only)

Open `http://reminder.this/` to see every reminder with its status. You can't
create reminders here — that's Claude or the CLI — but each card has a **Test**
button (fires the notification now to check it works, without changing the
reminder), a **Cancel** button (stops it firing, keeps the record), and a
**Delete** button (removes it for good).
...
[`CLAUDE.md`](CLAUDE.md). Tests are hermetic (`make test`): they use temp dirs
and a fixed clock, so they never touch your real reminders.
