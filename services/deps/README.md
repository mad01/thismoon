...
Open `http://deps.this/`. It shows the per-ecosystem counts, the active findings,
and an acknowledged section. From here you can:
...
and canned `go list` / OSV / lockfile fixtures and a fake notifier, so they never
hit the network or fire a real notification.
