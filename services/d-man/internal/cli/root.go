...
func init() {
	rootCmd.PersistentFlags().StringVar(&flagConfig, "config", defaultConfig(),
		"path to routes.toml (env DMAN_CONFIG)")
...
// defaultConfig resolves the routes file, honoring DMAN_CONFIG and falling back
// to ~/.config/d-man/routes.toml.
func defaultConfig() string {
	if v := os.Getenv("DMAN_CONFIG"); v != "" {
		return v
	}
	...
	return filepath.Join(home, ".config", "d-man", "routes.toml")
}
