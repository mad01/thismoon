...
whose HOME points nowhere useful). serve watches it with fsnotify and reloads
on content change or SIGHUP. Two sharp edges:
...
Routes are machine-private overlay config, so a freshly installed service has
no route until its overlay adds one. `d-man list` shows what is routed; a 502
means the route exists but the backend is down.

Every host fails at once: serve is not running, or something else holds :80.
`curl -s http://127.0.0.1/__this/sites.json` answers from d-man itself on any
host, so it tests the proxy without involving any backend.
...
the new build. If skew is still suspected, kill the serve process; KeepAlive
brings it back on the current binary.

## first moves

1. Split the fault: `curl -s http://127.0.0.1:<port>/` against the backend
   directly. Backend answers but `<name>.this` does not: it is d-man.
2. `curl -s http://127.0.0.1/__this/sites.json`: is serve answering :80.
3. `d-man list` and `d-man config`: does the route exist, and which routes
   file (loaded / missing / parse error) produced it.
4. `grep -A 30 'd-man managed' /etc/hosts`: is the managed block present and
   does it name the failing host.
5. Check /var/log/d-man/ for "reload failed, keeping current routes/hosts":
