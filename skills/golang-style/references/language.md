...
- A must-succeed primitive failing: the ID generators panic when `crypto/rand` fails (`present/internal/store/id.go`: `panic("present: crypto/rand failed: " + err.Error())`) — same pattern in `keep` and `reminder`.
