...
that silently never fires). Every concrete signal fails on the real config
— the `dotfiles-*` prefix covers repos that are name-exempt but
...
each on the wrong kind. No alias, no deprecation cycle.
...
wrong-exemption cost is the cheap kind — and naming the list after the
workflow fact, not the mechanism, is what keeps its reach self-limiting.
...
- commit-guard keeps `commit_guards[].always_allow` and takes no toggle
  key; a `direct_main_repos` entry does not exempt it — being a direct-main
  repo says nothing about work-hours discipline.
