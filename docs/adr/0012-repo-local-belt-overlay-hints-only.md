...
- **Hints only.** The overlay can configure hints about that repo — for
  commit-policy: an `exclude` opt-out (or opt-in overriding the machine's
  `exclude_repos`), a replacement `protected_branches` set, and a `message`
  appended to the advice. The worst a malicious or wrong file can do is
...
- Unknown hint ids and unknown keys in `.belt.yaml` are ignored rather
  than rejected — a repo may target a newer belt than the machine runs,
