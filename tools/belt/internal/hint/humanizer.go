...
func (h *Humanizer) Check(in Input) *Advice {
	...
	if raw, err := os.ReadFile(in.TranscriptPath); err == nil &&
		bytes.Contains(raw, humanizerNameKey) {
...
// say) from spending a session's one nudge on a local command. The rule is
// shared with the publish-internal-names guard via internal/mcptool.
func publishesText(tool string) bool {
	return mcptool.Publishes(tool)
}
