...
| `commit-policy` | `bash` | After a `git commit` lands on main/master of a repo not on `hints.commit-policy.allow_repos` (same canonical `host/owner/repo` patterns as the git-push-main guard), state the branch + PR policy with the exact recovery commands. Resolves the commit's repo dir from `git -C` (or the session cwd) via `guard.GitCommitDirs`, the branch via `git rev-parse --abbrev-ref HEAD`, and the identity via `guard.CanonicalRepoAt`. Silent on non-default branches, allowlisted repos, and repos with no resolvable origin — a scratch `git init` repo lives on main by design. |
