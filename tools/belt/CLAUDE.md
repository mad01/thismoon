...
belt/
  ...
  internal/
    ...
    config/           - belt config (profiles, internal_names, claude_settings, toggles) + ralph profiles fallback + gated Claude-settings deny list
...
`internal/config` treats `~/.config/belt/config.yaml` as the belt-owned surface: per-guard toggles, exclude paths, extra patterns, `profiles`, `claude_settings`, and the `internal_names` section (a legacy `config.toml` beside it is read only when the YAML file is absent, and a present-but-broken file of either format yields defaults rather than the other file). The config is standalone (docs/adr/0010): belt never reads another guard tool's file, and an absent `internal_names` section means an empty name set. Exactly two non-belt surfaces are read, both platform-level: `profiles` falls back to `~/.config/ralph/config.local.toml` (`ProfileSource` records which source won, shown by doctor), and the `permissions.deny` Bash entries of `~/.claude/settings.json` + `settings.local.json` are read live for the script guard (shared source of truth with the permission system), gated by `claude_settings.enabled` (default true). Every file is optional: `config.Load()` never errors, and a missing file yields zero values.
