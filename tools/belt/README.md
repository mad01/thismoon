...
- `publish-internal-names`: the same name set, checked on the way out to a public github.com remote instead of on the way into a file. On the `bash` event it covers `git push` (the pushed ref names plus the messages of the commits the remote does not have yet), `git commit -m`, `git tag`, branch creation, and the gh subcommands that write (pr, issue, release, repo, gist, label, api), reading any `--body-file` the call names. On the `external-text` event it covers the gh MCP tools the consuming repo routes to it: every string in the call except the owner/repo pair, so PR bodies, `head` branch names, labels, and `push_files` content all count. Deletions are allowed, `allow_repos` exempts a private companion repo, and `mode: soft` turns denials into warn events for rollout.
