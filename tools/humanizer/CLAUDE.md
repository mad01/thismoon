...
humanizer/
  ...
  internal/
    ...
    backend/         - LLM provider behind the judge pass (MAD-342): env-driven selection (backend.go) over one OpenAI-compatible chat client (chat.go) with two provider constructors, LiteLLM proxy (litellm.go) and OpenRouter (openrouter.go); vertex/anthropic planned
...
- `humanizer_judge(text, backend?, model?)` → `{verdict, confidence, signals[], summary, backend, model}`. Holistic whole-passage AI/human judgment through the LLM backend (`internal/backend`): auto-detects from env (`LITELLM_BASE_URL` → litellm, default model `claude-haiku-4-5-20251001`; else `OPENROUTER_API_KEY` → openrouter, default model `anthropic/claude-haiku-4.5`; `HUMANIZER_MODEL` overrides). The one networked tool — needs egress plus the provider env in the server env; under a denying sandbox it errors and the caller falls back to the `humanizer judge` CLI.
...
- **humanizer_judge is the one networked MCP tool.** Backend selection is env-driven (`HUMANIZER_BACKEND` override, else the first configured provider: a LiteLLM proxy via `LITELLM_BASE_URL` wins over an OpenRouter key; vertex/anthropic are MAD-342 follow-ups). Both providers share one OpenAI-compatible chat client (`chat.go`), so a LiteLLM deployment differs only in host, optional bearer key, and model alias. The LiteLLM default is the Anthropic-native Haiku 4.5 id; a proxy whose `model_list` aliases it differently needs `HUMANIZER_MODEL`, and a pasted `/v1` suffix on the base URL is trimmed. The consuming repo's seatbelt must allow outbound TLS to the provider host and inject the provider env (`OPENROUTER_API_KEY`, or `LITELLM_BASE_URL` plus `LITELLM_API_KEY`; dotfiles ADR-0020), or the tool errors while everything else stays offline.
