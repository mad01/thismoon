...
| `prepare` | off | Boolean. Registers the page's text with speak on connect, so parts are synthesized ahead of playback and the page shows how far that got |
| `name` | page title | The document name speak files the page under; it names the downloads |
...
Speech is fetched in parts that start at one sentence and grow to a few hundred characters, two ahead of the one playing; on a page whose sections list their part keys in `data-ra-parts` (prepared mode stamps them), the parts are prepared audio from `GET {endpoint}/audio/{key}`. The playing part is highlighted with `.wk-ra-active` (background `--ra-highlight`), on its sentence spans (`.wk-ra-sentence`) or on the `data-ra-chunk` blocks it reads, and the play button pulses (`.wk-ra-wait`) while that part is still loading. Starting a second section stops the first. While a section plays, a restart button appears beside its play/pause control to rewind to the section start. Esc clears selection playback.

With `prepare`, the element registers the page with speak itself (`POST {endpoint}/read`, one list of block texts per target section), stamps the keys speak answers onto the blocks (`data-ra-chunk`) and sections (`data-ra-parts`), and renders a status bar inside itself: how many parts are ready, with "Prepare all", "Retry failed" and "Download page audio" buttons. Each section gets a state badge (`wk-badge.wk-ra-state`) beside its play buttons, with "Retry" and "Download" as they apply. The status is polled every 2s while parts are in progress, backing off to once a minute while speak doesn't answer. If registration fails the page reads live as before, with no toast. If speak later forgets the document (it restarted), the page registers again; should that fail too, the stamps come off, one toast says so, and the page reads live from then on.
